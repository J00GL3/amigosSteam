...
export default function TablaPrueba(props) {
  ...
  return (
    <div >
      {/* {data.length > 0 ? (<h2 style={{textAlign: "left", margin: "20px 0 5px 0"}}>Tabla Relaciones</h2>): (<></>)} */}
      <table style={{margin: "20px 0 5px 0"}}>
        <thead>
          <tr>
            {data[0].map((headerItem, index) =>
              index == 0 ? (<th key={index}>
                <p style={{display:"flex", justifyContent:"flex-end"}}>Jugador</p>
              </th>) : 
              (<th key={index}>
            ...
            <tr key={rowIndex}>
              {
                  ...
                  <td key={cellIndex} style={(cell.amistad && cell.totalComunes > 0) ? {backgroundColor:"#16abff33"}:{backgroundColor: ""}}>
                    <div style={{display:"flex", justifyContent: "flex-end", textAlign: "left"}}>
                    ...
                    <p style={{display:"flex",alignItems: "center", justifyContent: "center", cursor: "pointer"}} onClick={() => enviarData([cell.jugadora, cell.jugadorb, cell.arrayComunes])}>{cell.totalComunes}</p>
